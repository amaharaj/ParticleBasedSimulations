...
while { $x < $L } {
	for { set i 0 } { $i < $Nwallseg } { incr i } {
		if { $x < $L} {
         part [expr $npart + $npartwall] pos $x [expr $ywall + $space] 0 fix
         incr npartwall
			part [expr $npart + $npartwall] pos $x [expr $ywall + $space*2] 0 fix
         incr npartwall
         part [expr $npart + $npartwall] pos $x [expr $ywall + $space*3] 0 fix
         incr npartwall
         part [expr $npart + $npartwall] pos $x [expr $ywall + $space*4] 0 fix
         incr npartwall
         part [expr $npart + $npartwall] pos $x [expr $ywall1 + $space + $separation] 0 fix
			incr npartwall
         part [expr $npart + $npartwall] pos $x [expr $ywall1 + $space*2 + $separation] 0 fix
         incr npartwall
         part [expr $npart + $npartwall] pos $x [expr $ywall1 + $space*3 + $separation] 0 fix
         incr npartwall
    #     part [expr $npart + $npartwall] pos $x [expr $ywall1 + 15] 0 fix
     #    incr npartwall
        # part [expr $npart + $npartwall] pos $x [expr $ywall1 + 30] 0 fix
         #incr npartwall
...
#while { $x < $L } {
 #  set x [expr $x + $Nwallseg]
  # for { set y 0 } { $y < $Nwallseg } { incr y } {
   ...
   #   part [expr $npart + $npartwall] pos $x [expr ($ywall - 1.5*$Nwallseg + 2*$a) - $y - 2] 0 fix
    #  incr npartwall
   #}
   #incr a
   #set x [expr $x + $Lgap]
...
for { set t 0 } { $t < $trun } { incr t } {
	...
	imd positions
   for { set i 0 } { $i < $npart } { incr i } {
      part $i ext_force [expr cos(2*$t) - 0.3] 1.0 0
   }
}
